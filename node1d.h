...
//     longer linked list.
//
//   node* list_insert(node* cursor_ptr, const node::value_type& entry)
...
//
//   node* list_append(node* cursor_ptr, const node::value_type& entry)
//     Precondition: cursor_ptr points to a node in a linked list.
...
namespace project_stringlist
{
	...
	node* list_head_insert(node*&, const node::value_type&);
	node* list_insert(node*, const node::value_type&);
	node* list_append(node*, const node::value_type&);
