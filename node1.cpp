...
#include <cstdlib>    // Provides nullptrptr and size_t
...
namespace project_stringlist
{
		...
		// Library facilities used: cstdlib
	{
		...
		answer = 0;
		for (cursor = head_ptr; cursor != nullptr; cursor = cursor->nlink())
			++answer;
	...
	{
		if (head_ptr == nullptr)
			head_ptr = new node(entry, head_ptr, nullptr);
		else {
			head_ptr = new node(entry, head_ptr, nullptr);
	...
	node* list_insert(node* node_ptr, const node::value_type& entry)
	{
		node *insert_ptr;
	...
	node* list_append(node* node_ptr, const node::value_type& entry)
	{
		node *insert_ptr;
		assert(node_ptr != nullptr);
	...
	void list_head_remove(node*& head_ptr)
	{
		...
		if (head_ptr->nlink() != nullptr)
			head_ptr->nlink()->set_plink(nullptr);
	...
	{
		assert(cursor_ptr != nullptr);

		if (cursor_ptr->plink() != nullptr) {
			cursor_ptr->plink()->set_nlink(cursor_ptr->nlink());
		}

		if (cursor_ptr->nlink() != nullptr)
	...
	void list_piece(const node* start_ptr, const node* end_ptr, node*& head_ptr, node*& tail_ptr)
	{
		...
		while (start_ptr != end_ptr && start_ptr != nullptr) {
